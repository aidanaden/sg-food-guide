...
const SliderThumb: React.FC<SliderPrimitive.Thumb.Props> = ({ className, ...props }) => (
  <SliderPrimitive.Thumb
    ...
    className={cn(
      "border-ring ring-ring bg-primary-foreground relative block size-5 shrink-0 cursor-grab rounded-full border transition-colors transition-shadow select-none after:absolute after:-inset-2 hover:ring-2 focus-visible:ring-2 focus-visible:outline-hidden active:ring-2 disabled:pointer-events-none disabled:opacity-50 data-[dragging]:cursor-grabbing",
      className,
    )}
    {...props}
  />
