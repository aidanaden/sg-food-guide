...
export async function runScheduledCommentSuggestionSync(
  args: ScheduledCommentSyncArgs,
): Promise<void> {
  ...
  if (syncResult.value.status !== "success") {
    console.error(
      "Scheduled comment suggestion sync finished with non-success status.",
      syncResult.value,
    );
  } else {
