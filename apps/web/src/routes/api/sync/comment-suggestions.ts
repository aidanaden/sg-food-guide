...
} from '../../../server/sync/comment-suggestion-sync';
  ...
  server: {
    handlers: {
      POST: async ({ request, context }) => {
        ...
        if (!payload.success) {
          return json({ error: 'Invalid comment suggestion sync request payload.' }, 400);
        }
        ...
        const mode = payload.data.mode as CommentSuggestionSyncMode | undefined;
        const summary = await runCommentSuggestionSync({
          env: envResult.value,
          triggerSource: 'api:manual:comment-suggestions',
          modeOverride: mode,
      ...
      GET: async ({ request, context }) => {
        ...
        if (!payload.success) {
          return json({ error: 'Invalid comment suggestion sync query payload.' }, 400);
        }
        ...
        const summary = await runCommentSuggestionSync({
          env: envResult.value,
          triggerSource: 'api:get:comment-suggestions',
