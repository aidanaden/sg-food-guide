...
export async function renderPage() {
  ...
  return `
    <div id="app"></div>
    <script>window.__SERVER_POSTS__ = ${JSON.stringify(data)}</script>
  ` // Bypasses Query cache
}

// client-side
function PostsPage() {
  const posts = window.__SERVER_POSTS__ as Post[]
...
## Good Example: TanStack Router/Start (Preferred)
...
## Good Example: Manual SSR Setup (Non-Router or Custom SSR)

Use this pattern when you are not using TanStack Router + `@tanstack/react-router-ssr-query`.
For TanStack Router/Start projects, prefer `setupRouterSsrQueryIntegration` and loader-based prefetch.
...
- For TanStack Router + Query apps, prefer integration package setup over manual dehydrate/hydrate boilerplate
