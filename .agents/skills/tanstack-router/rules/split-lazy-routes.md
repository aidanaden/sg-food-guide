...
export const Route = createFileRoute('/dashboard')({
  loader: async ({ context }) => {
    await context.queryClient.ensureQueryData(dashboardQueries.stats())
  },
...
## Using getRouteApi in Lazy Components (Route-Managed Data)
...
function PostPage() {
  ...
  // This pattern is for route-managed loader data (no TanStack Query ownership)
...
## Using getRouteApi in Lazy Components (Query-Backed Data)
...
function PostPage() {
  // Use route API for params/search typing
  const { postId } = route.useParams()
  // Read server-state from TanStack Query cache
...
- For Query-backed routes, use loaders to prefetch and Query hooks to read data in components
- In Query-backed routes, use `getRouteApi()` for params/search and Query hooks for data
