...
export const Route = createFileRoute('/posts')({
  loader: async ({ context: { queryClient } }) => {
    // Warm Query cache; keep server-state ownership in Query
...
function DashboardPage() {
  ...
  const { data } = useSuspenseQuery(dashboardQueries.forUser(auth.user!.id))
...
- In Query-backed routes, loaders should warm Query cache and components should read data via Query hooks
