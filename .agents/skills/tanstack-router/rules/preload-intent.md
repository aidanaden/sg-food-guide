...
## Preload Options (Router-Managed Cache Mode)
...
const router = createRouter({
  ...
  defaultPreloadStaleTime: 30000, // 30s - for router-managed loader cache
})
...
- Use non-zero `preloadStaleTime` for router-managed cache mode
- Use `defaultPreloadStaleTime: 0` when TanStack Query manages server-state cache
