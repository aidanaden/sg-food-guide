...
// 5. useLoaderData hook (route-managed loader data)
const data = useLoaderData({ from: '/posts/$postId' })  // Knows loader return type

// 6. Query-backed route data (use Query for server-state reads)
...
- In Query-backed routes, keep Router hooks for typing/navigation and Query hooks for server-state reads
