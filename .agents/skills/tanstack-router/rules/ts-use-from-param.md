...
If the route is Query-backed, use these hooks for route typing (params/search), but read server-state via TanStack Query hooks (`useQuery`/`useSuspenseQuery`) rather than `useLoaderData`.
...
function PostPage() {
  // Route-managed loader data (no Query ownership)
...
function PostComponent() {
  ...
  const { post } = useLoaderData({ from: Route.fullPath }) // Route-managed data
...
## Query-Backed Route Variant

```tsx
function PostComponent() {
  // Keep route typing from Router
  const { postId } = useParams({ from: '/posts/$postId' })

  // Read server-state from Query cache
...
- In Query-backed routes, keep `useLoaderData` for route-owned metadata and read server-state via Query hooks
